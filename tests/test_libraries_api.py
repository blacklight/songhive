...
async def _add_library_tracks(db_session, library: Library, owner, *visibilities):
    """Create tracks and add them to ``library`` with the given visibilities."""
...
@pytest.mark.asyncio
async def test_share_grant_library_cascades_to_tracks(
    client, db_session, sample_libraries, regular_user, other_user, auth_headers
):
    """Sharing a private library gives the recipient access to its tracks."""
    library_data = next(lib for lib in sample_libraries if lib["visibility"] == "private")
    library = await db_session.get(Library, library_data["id"])
    tracks = await _add_library_tracks(db_session, library, regular_user, Visibility.PRIVATE)

    share = client.post(
        "/api/v1/shares",
        json={"item_type": "library", "item_id": str(library.id), "user_id": str(other_user.id)},
        headers=auth_headers(regular_user),
    )
    ...
    assert library_after.status_code == 403
