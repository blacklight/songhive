"""
Tests for adding tracks, albums and artists to a library.
"""
...
@pytest.mark.asyncio
async def test_add_track_to_library(client, regular_user, db_session, auth_headers):
    """A track can be added to a library by its owner."""
    ...
    assert link.added_by_id == regular_user.id


@pytest.mark.asyncio
async def test_add_album_to_library(client, regular_user, db_session, auth_headers):
...
@pytest.mark.asyncio
async def test_add_artist_to_library(client, regular_user, db_session, auth_headers):
...
@pytest.mark.asyncio
async def test_add_tracks_to_library_forbidden_for_non_owner(
    client, regular_user, other_user, db_session, auth_headers
):
    """A user that does not manage the library receives a 403."""
    artist = await _make_artist(db_session)
    track = await _make_track(db_session, artist, owner=regular_user, visibility=Visibility.PUBLIC.value)
    library = await _make_library(db_session, regular_user)
...
@pytest.mark.asyncio
async def test_add_inaccessible_tracks_to_library_are_skipped(
    client, regular_user, other_user, db_session, auth_headers
):
    """Tracks the user cannot access are not added to the library."""
    artist = await _make_artist(db_session)
    album = await _make_album(db_session, artist, owner=regular_user)
    ...
    result = await db_session.execute(select(LibraryTrack.track_id).where(LibraryTrack.library_id == library.id))
    stored = set(result.scalars().all())
    assert stored == {accessible.id}


@pytest.mark.asyncio
async def test_add_tracks_to_library_creates_audit_log(client, regular_user, db_session, auth_headers):
    """Adding tracks writes a library_track.add AuditLog row."""
    artist = await _make_artist(db_session)
    track = await _make_track(db_session, artist, owner=regular_user, visibility=Visibility.PUBLIC.value)
    library = await _make_library(db_session, regular_user)
    ...
    log = await db_session.scalar(
        select(AuditLog)
        .where(
            AuditLog.action == "library_track.add",
