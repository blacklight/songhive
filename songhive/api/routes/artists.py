...
def _artist_tags(artist) -> List[str]:
    """Return loaded tag names, avoiding a lazy load."""
    if not _is_loaded(artist, "tags"):
        return []
    return [h.name for h in artist.tags]
...
@router.post("/{artist_id}/tags", response_model=ArtistResponse)
async def add_artist_tags(
    ...
    """Add tags to an artist (admin only)."""
    artist = await music.get_artist(db, artist_id)
    if artist is None:
    ...
    artist = await music.get_artist(db, artist_id, include=set(include.values) | {"tags"})
    await audit.log_action(
        db,
        actor_id=current_user.id,
        action="tag.add",
...
async def remove_artist_tag(
    ...
    """Remove a tag from an artist (admin only)."""
    artist = await music.get_artist(db, artist_id)
    if artist is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Artist not found")
