...
def _generate_jti() -> str:
    """Return a URL-safe random JWT id."""
...
def create_access_token(
    ...
    payload = {
        "sub": user_id,
        "jti": jti if jti is not None else _generate_jti(),
...
def _decode_access_token(token: str, secret_key: str, verify_exp: bool = True) -> Optional[dict]:
...
def decode_access_token(token: str, secret_key: str) -> Optional[str]:
    ...
    payload = _decode_access_token(token, secret_key, verify_exp=True)
...
def get_access_token_jti(token: str, secret_key: str) -> Optional[str]:
    """Return the ``jti`` claim from an access token without verifying expiry."""
    payload = _decode_access_token(token, secret_key, verify_exp=False)
    if not isinstance(payload, dict):
        return None
