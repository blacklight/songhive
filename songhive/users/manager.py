...
async def update_profile(session: AsyncSession, user: User, updates: Dict[str, Any]) -> User:
    ...
    if "profile_visibility" in updates:
        profile_visibility = updates["profile_visibility"]
...
async def _remove_user_references(session: AsyncSession, user: User) -> None:
    ...
    await session.execute(delete(PodcastSyncEvent).where(PodcastSyncEvent.user_id == user.id))
    await session.execute(delete(PodcastSyncConfig).where(PodcastSyncConfig.user_id == user.id))
